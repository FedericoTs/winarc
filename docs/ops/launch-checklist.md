...
**Every `eas` command runs from `apps/mobile`, never from the repository root.** The root is a pnpm workspace, not an Expo app. Run there, `eas build` finds no `app.json`, invents a new app called `winarc` with `com.federicots.winarc` identifiers, writes a root `eas.json` and `app.json`, links them to the same EAS project, and uploads a build that can never work. If that has happened: `git status` shows the two files as untracked, so `del app.json eas.json` at the root, then carry on below. The builds it queued can be cancelled at expo.dev, and the keystore it created for the wrong identifier can be ignored. From the root, `pnpm eas <anything>` and `pnpm ios:dev` run the command in the right place.
...
- **Build the dev client.** `eas build --profile development --platform ios` from `apps/mobile`, or `pnpm ios:dev` from the root. The first run logs in to Apple, offers to generate a distribution certificate and provisioning profile, and lets EAS manage both; say yes to all of it. Expect 10 to 25 minutes including queue. When it finishes, open the build link on the phone and install. Expo Go is not a shortcut here: HealthKit, Health Connect and Sign in with Apple are native modules that Expo Go does not carry, so the app only runs as this build.
